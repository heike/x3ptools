...
#' 
#' Select a circle on the surface of an x3p file by identifying three points along the circumference.
#' @param x3p x3p file
#' @param col character value of the selection color
...
x3p_circle_select <- function(x3p, col = "#FF0000", update=TRUE) {
  ...
  selected <-  identify3d(xidx, yidx ,z, n=3)
...
#' 
#' Use the right mouse button (double-finger on track pad) to identify to points on the
#' active rgl device (this rgl device should show the surface of an x3p object). These points define the beginning and end  of a line segment.
...
#' \dontrun{
#' if (interactive) {
#'   x3p <- x3p_read("~/Documents/Data/Klein/STL-files GelSight/standard-bullet-land.x3p")
#'   x3p %>% image_x3p(size=dim(x3p$surface.matrix), multiply=1, zoom=1)
...
x3p_extract_profile <- function(x3p, col = "#FF0000", update=TRUE, line_out=TRUE) {
  ...
  xidx <- rep(x, length(y))
  yidx <- rep(y, each=length(x))
  selected <-  identify3d(xidx, yidx ,z, n=2)
#  browser()
  ...
  #  x3p <- x3p_df %>% df_to_x3p()
  if (is.null(x3p$mask)) x3p <- x3p %>% x3p_add_mask()
  x3p$mask[matrix(on_line, nrow = dim(x3p$mask)[1], byrow=TRUE)] <- col
